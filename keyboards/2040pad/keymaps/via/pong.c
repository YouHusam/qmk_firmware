...
void ball_render_func(uint8_t *data, void *user_args) {
    ...
    pre_ball_byte = *data & ball;
    // *data |= ball;
}

uint16_t get_oled_index(uint8_t x, uint8_t y) {
    return x + (y ) * OLED_DISPLAY_WIDTH;
}
...
void pong_frame() {
    ...
    if (FLAG_CHECK(update_flag, update_net)){
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 0));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 1));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 2));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 3));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 4));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 5));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 6));
        oled_write_raw_byte(0x60, get_oled_index(NET_COLUMN, 7));
    ...
    if (FLAG_CHECK(update_flag, update_player_left)) {
        ...
        oled_write_raw_byte(0x00, get_oled_index(0, line - 1));
        oled_write_raw_byte(0xFF << sub_pos, get_oled_index(0, line));
        oled_write_raw_byte(~(0xFF << sub_pos), get_oled_index(0, line + 1));
    ...
    if (FLAG_CHECK(update_flag, update_player_right)) {
        ...
        oled_write_raw_byte(0x00, get_oled_index(OLED_DISPLAY_WIDTH - 1, line - 1));
        oled_write_raw_byte(0xFF << sub_pos, get_oled_index(OLED_DISPLAY_WIDTH - 1, line));
        oled_write_raw_byte(~(0xFF << sub_pos), get_oled_index(OLED_DISPLAY_WIDTH - 1, line + 1));
    ...
    if (FLAG_CHECK(update_flag, update_ball)) {
        ...
        // erase the previous position of the ball
        {
            ...
            oled_write_raw_byte(pre_ball_byte, get_oled_index(prev_ball_pos.x, line)); // revert the display to its previous value
        }
        ...
        // draw the new position of the ball
        {
            ...
            ball_render_func(&ball_byte, &pre_ball_byte);
            oled_write_raw_byte(ball_byte, get_oled_index(ball.position.x, line));
